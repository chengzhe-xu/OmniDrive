...
class PETRHeadv2(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRHeadv2):
        ...
        super(PETRHeadv2, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRHeadv2:
            convert_dict = {
    ...
    def forward(self, img_metas, pos_embed, **data):
        ...
        B, N, C, H, W = x.shape
        num_tokens = H * W
        memory = x.permute(0, 1, 3, 4, 2).reshape(B*N, num_tokens, C)

        memory = self.input_projection(memory)
        tgt = self.query_embedding.weight.unsqueeze(0).repeat(B*N, 1, 1)
        query_pos = torch.zeros_like(tgt)
            
        # transformer here is a little different from PETR
        outs_dec = self.transformer(tgt, memory, query_pos, None)

        outs_dec = torch.nan_to_num(outs_dec)
    
        if self.output_dims is not None:
            vlm_memory = self.output_projection(outs_dec[-1]).reshape(B, -1, self.output_dims)
