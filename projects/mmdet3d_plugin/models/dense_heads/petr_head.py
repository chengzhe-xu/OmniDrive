...
class PETRHead(AnchorFreeHead):
    ...
    def forward(self, img_metas, pos_embed, **data):
        ...
        tgt = self.query_embedding.weight.unsqueeze(0).repeat(B, 1, 1)
        query_pos = torch.zeros_like(tgt)
            
        # transformer here is a little different from PETR
        outs_dec = self.transformer(tgt, memory, query_pos, pos_embed)

        outs_dec = torch.nan_to_num(outs_dec)
    
        if self.output_dims is not None:
            vlm_memory = self.output_projection(outs_dec[-1])
